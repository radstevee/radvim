...
local SidebarOpen = {
  text = function(buffer)
    ...
    if (open and buffer.is_readonly) or (buffer.is_first and not open) then
      return string.format(' %s ', open and icons.menu.actions.outline.left or icons.menu.actions.outline.right)
    end
    return ''
  end,
  fg = function(cx)
...
local Devicon = {
  ...
  fg = function(buffer)
    return (mappings.is_picking_focus() and 'DiagnosticWarn') or (mappings.is_picking_close() and 'DiagnosticError') or buffer.devicon.color
  end,
  italic = function(_)
...
local opts = {
  ...
  buffers = {
    ...
    filter_visible = function(buf)
      if string.match(buf.filename, 'neo-tree') then
        return false
      end
      return true
    end,
  ...
  components = {
    {
      text = function(buffer)
        return buffer.is_focused and separators.left or icons.separators.bar.left
      end,
      fg = function(buffer)
        return buffer.is_focused and 'Directory' or '#0d0f18' -- TODO: use hlgroup
      end,
      bg = function(buffer)
        return buffer.is_focused and groups.bg_active or groups.bg
      end,
  ...
  tabs = {
    ...
    components = {
      Padding,
      {
        ...
        fg = function(tab)
          return tab.is_active and 'Directory' or '#0d0f18'
        end,
        bg = function(tab)
      ...
      },
      {
        ...
        fg = function(tab)
          return tab.is_focused and 'Directory' or '#0d0f18'
        end,
        bg = function(tab)
          return tab.is_focused and groups.bg_active or groups.bg
        end,
